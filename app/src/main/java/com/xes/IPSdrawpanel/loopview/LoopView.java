package com.xes.IPSdrawpanel.loopview;
...
public class LoopView extends View {
    ...
    private void measureTextWidthHeight() {
        ...
        for (int i = 0; i < items.size(); i++) {
            String s1 = items.get(i).getCityName();
...
//    }
    ...
    public final void setItems(List<areaBean> items) {
        this.items = items;
        remeasure();
        invalidate();
    }
...
//        Log.i("test", (new StringBuilder("preCurrentIndex=")).append(preCurrentIndex).toString());
        ...
        while (k1 < itemsVisible) {
            ...
            if (isLoop) {
                ...
                as[k1] = items.get(l1).getCityName();
            } else if (l1 < 0) {
                as[k1] = "";
        ...
        while (j1 < itemsVisible) {
            ...
            // 弧度转换成角�?(把半圆以Y轴为轴心向右�?90度，使其处于第一象限及第四象�?
            ...
            if (angle >= 90F || angle <= -90F) {
                ...
                if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
                    ...
                    for(int position = 0;position < items.size();position++){
                    	if(as[j1].equals(items.get(position).getCityName())){
